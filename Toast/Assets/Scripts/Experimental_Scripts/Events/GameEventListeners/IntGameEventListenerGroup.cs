...
public class IntGameEventListenerGroup : MonoBehaviour
{
    [SerializeField]
    List<IntGameEventListener> gameEventListeners;
...
public class IntGameEventListener
{
    ...
    [SerializeField, Label("(Int) GameEvent"), AllowNesting]
    private IntGameEvent gameEvent; // the GameEvent this listener will subscribe to
    [SerializeField]
    private IntUnityEvent response; // the response that will be invoked
    ...
    public void OnEventRaised(int value) // when the event is raised, invoke the response
    {
